...
ID3D12GraphicsCommandList* PostEffectMixed::commandList = nullptr;

VertexPosUv PostEffectMixed::vertices[4] = {};

VertexPosUv* PostEffectMixed::vertMap = nullptr;

Microsoft::WRL::ComPtr<ID3D12Resource> PostEffectMixed::vertBuff = nullptr;	//頂点バッファ

//頂点バッファビューの作成
D3D12_VERTEX_BUFFER_VIEW PostEffectMixed::vbView{};

Microsoft::WRL::ComPtr<ID3D12Resource> PostEffectMixed::texBuff;
//深度バッファ
Microsoft::WRL::ComPtr<ID3D12Resource> PostEffectMixed::depthBuff;
//RTV用のデスクリプタヒープ
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> PostEffectMixed::descHeapRTV;
//DSV用のデスクリプタヒープ
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> PostEffectMixed::descHeapDSV;

Microsoft::WRL::ComPtr<ID3D12PipelineState> PostEffectMixed::pipelineState;
Microsoft::WRL::ComPtr<ID3D12RootSignature> PostEffectMixed::rootSignature;
...
void PostEffectMixed::Draw(uint32_t texHandle)
{
	...
	//SRVヒープの先頭ハンドルを取得（SRVを指しているはず）
		D3D12_GPU_DESCRIPTOR_HANDLE srvGpuHandle = PostEffectCommon::descHeapSRV->GetGPUDescriptorHandleForHeapStart();
		UINT incrementSize = PostEffectCommon::device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		srvGpuHandle.ptr += incrementSize * texHandle;
